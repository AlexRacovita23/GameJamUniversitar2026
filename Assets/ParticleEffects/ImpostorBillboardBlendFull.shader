Shader "ParticleEffects/ImpostorBillboardBlendFull"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float row0 = clamp(floor(grid.y),     0, _FramesPerAxis - 1);
                float row1 = clamp(floor(grid.y) + 1, 0, _FramesPerAxis - 1);
                float blendX = frac(grid.x);
                float blendY = frac(grid.y);

                float2 uv00 = float2(col0*frameSize, row0*frameSize) + i.quadUV*frameSize;
                float2 uv10 = float2(col1*frameSize, row0*frameSize) + i.quadUV*frameSize;
                float2 uv01 = float2(col0*frameSize, row1*frameSize) + i.quadUV*frameSize;
                float2 uv11 = float2(col1*frameSize, row1*frameSize) + i.quadUV*frameSize;
                ...
                fixed4 col = lerp(lerp(c00, c10, blendX),
                                  lerp(c01, c11, blendX),
                                  blendY) * i.color * _AmbientTint;
                clip(col.a - _Cutoff);
